...
<template>
  <main class="pt-5 lg:pt-7 pb-16 lg:pb-24">
    <AppContainer>
      <CommonBreadcrumbs :loading="categoriesLoading" :items="breadcrumbs" class="mb-2 lg:mb-4" />

      <!-- Category Name -->
      <h1 class="text-2xl lg:text-5xl font-semibold uppercase tracking-wide text-slate-950">
      ...
      <!-- Subcategories -->
      <div v-if="subcategories.length > 0" class="mt-2 lg:mt-3">
        <ul class="flex flex-wrap gap-x-4 gap-y-1 lg:gap-x-6 lg:gap-y-2 text-sm lg:text-base">
          <li
            v-for="subcategory in subcategories"
            :key="subcategory.id"
            class="flex items-start gap-1.5 text-slate-600"
          >
            <Icon icon="mdi:arrow-right-bottom" class="w-4 h-4 shrink-0 lg:mt-0.5" />
